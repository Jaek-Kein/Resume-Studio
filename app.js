(() => {
  ...
  function bind(){
    ...
    app.querySelectorAll('[data-action]').forEach(b=>{const a=b.dataset.action;if(a==='visibility')return;b.addEventListener('click',async()=>{
      ...
      if(a==='add'){state.resume[b.dataset.key].push(addDefaults[b.dataset.key]());scheduleSave();render();}
      if(a==='remove'){state.resume[b.dataset.key].splice(Number(b.dataset.index),1);scheduleSave();render();}
      if(a==='template'){state.preferences.template=b.dataset.value;scheduleSave();render();}
      if(a==='remove-photo'){state.resume.photo='';scheduleSave();render();}
      if(a==='preset'){const on=presets[b.dataset.value]||[];state.preferences.hiddenSections=defaultOrder.filter(k=>!on.includes(k));scheduleSave();render();}
    ...
    const photo=document.querySelector('#photoInput');if(photo)photo.onchange=e=>{const f=e.target.files?.[0];if(!f)return;if(f.size>3*1024*1024){alert('사진은 3MB 이하 파일을 사용해 주세요.');return;}const r=new FileReader();r.onload=()=>{state.resume.photo=r.result;scheduleSave();render();};r.readAsDataURL(f);};
    const ji=document.querySelector('#jsonInput');if(ji)ji.onchange=e=>handleImport(e.target.files?.[0]);
    app.querySelectorAll('[data-action=visibility]').forEach(x=>x.onchange=e=>{const k=x.dataset.key;if(e.target.checked)state.preferences.hiddenSections=state.preferences.hiddenSections.filter(v=>v!==k);else if(!state.preferences.hiddenSections.includes(k))state.preferences.hiddenSections.push(k);scheduleSave();render();});
    app.querySelectorAll('.sort-item').forEach(x=>{x.ondragstart=()=>{dragKey=x.dataset.section;x.classList.add('dragging');};x.ondragend=()=>{dragKey=null;x.classList.remove('dragging');};x.ondragover=e=>e.preventDefault();x.ondrop=e=>{e.preventDefault();const to=x.dataset.section;if(!dragKey||dragKey===to)return;const a=state.preferences.sectionOrder,from=a.indexOf(dragKey),target=a.indexOf(to);a.splice(from,1);a.splice(target,0,dragKey);scheduleSave();render();};});
  }
  render();
})();
