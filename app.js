(() => {
  ...
  const fontPresets = {
    ...
    pretendard:{label:'Pretendard', stack:'Pretendard, "Noto Sans KR", sans-serif', url:'./media/fonts/pretendard.css'},
  ...
  function collectionEntry(type,x){
    ...
    if(type==='project') return `<div class="entry"><div class="entry-head project-head"><div class="project-identity"><strong>${esc(x.name||'프로젝트명')}</strong><span>${esc(x.role||'')}</span>${x.link?`<div class="entry-link">${link(x.link)}</div>`:''}</div><div class="project-meta"><time>${esc([x.start,x.end].filter(Boolean).join(' – '))}</time>${x.techStack?.trim()?`<span class="project-tech-stack">${esc(x.techStack)}</span>`:''}</div></div>${description(x.description)}</div>`;
  ...
  function renderSection(k){
    ...
    if(k==='military'){const m=d.military; const vals=[m.status,m.branch,m.rank,m.specialty,[m.start,m.end].filter(Boolean).join(' – '),m.dischargeReason].filter(Boolean);return vals.length?`<section>${sectionHead(k)}<p style="margin:0">${esc(vals.join(' · '))}</p></section>`:'';}
  ...
  function contentHTML(){ const d=state.resume; const visible=k=>!state.preferences.hiddenSections.includes(k); let html=`<div class="form-section"><h3>기본 정보</h3><div class="photo-row"><div class="photo-thumb">${d.photo?`<img src="${d.photo}">`:'<span>사진</span>'}</div><div><label class="upload-btn">사진 선택<input id="photoInput" type="file" accept="image/*"></label>${d.photo?'<button class="text-btn" data-action="remove-photo">사진 제거</button>':''}<p class="field-help">JPG/PNG, 3MB 이하. ATS형에서는 자동 숨김됩니다.</p></div></div><div class="grid2">${field('이름','resume.name',d.name)}${field('영문 이름','resume.englishName',d.englishName)}</div><div class="grid2">${field('직무 / 헤드라인','resume.title',d.title)}${field('국적 (선택)','resume.nationality',d.nationality)}</div><div class="grid2">${field('성별 (선택)','resume.gender',d.gender)}</div><p class="field-help">오른쪽 상단 항목 · 화살표로 입력 및 표시 순서를 바꿀 수 있습니다.</p><div class="basic-fields">${state.preferences.contactOrder.map((key,i)=>{const [label,type='text']=contactFields[key];return `<div class="basic-field" data-contact-field="${key}">${field(label,'resume.'+key,d[key],type)}<div class="basic-field-actions"><button type="button" class="ghost" data-action="move-contact" data-key="${key}" data-direction="-1" aria-label="${label} &#50526;&#51004;&#47196; &#51060;&#46041;" ${i===0?'disabled':''}>&uarr;</button><button type="button" class="ghost" data-action="move-contact" data-key="${key}" data-direction="1" aria-label="${label} &#46244;&#47196; &#51060;&#46041;" ${i===state.preferences.contactOrder.length-1?'disabled':''}>&darr;</button></div></div>`;}).join('')}</div></div>`;
    for(const k of state.preferences.sectionOrder){
      ...
      if(k==='objective')html+=`<div class="form-section"><h3>직무 목표</h3>${area('Career Objective','resume.objective',d.objective,4)}</div>`;
  ...
  async function printResume(){
    ...
    exportingPDF=true;
    const buttons=[...app.querySelectorAll('[data-action="print"]')];
    buttons.forEach(button=>{button.disabled=true;button.textContent='PDF 생성 중…';});
    try{
      ensureFont();await document.fonts.ready;
      window.refreshPageGuides();await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
      await window.downloadResumePDF({title:state.resume.name||'resume'});
    }catch(error){console.error(error);alert(`PDF를 저장하지 못했습니다. ${error.message}`);}
    finally{exportingPDF=false;buttons.forEach(button=>{button.disabled=false;button.textContent='PDF 다운로드';});}
  }
